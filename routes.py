...
def reset():
    ...
    if form.validate_on_submit():
        ...
        if user:
            pw_hash = bcrypt.generate_password_hash(form.new_password.data)
            update_old_pwd = (update(User).where(User.password == form.password.data).
            values(password=pw_hash))
            db.session.execute(update_old_pwd)
